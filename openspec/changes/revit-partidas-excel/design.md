...
**The export (PR 32).** The command asks extraction for the layers of the categories the criteria take off by layer, none without a criteria file. The export measures each of their elements by its materials, each line coded by its material, or, when its materials do not account for it, whole under its own code with the reason; an opening near the threshold is flagged once for the host. A layer its type leaves without a material is measured as the category's material ("Default Wall" on Snowdon's Alley wall), and the run says so once for the type. The dialog describes a layered criterion (which functions are in m3, how an added-back opening returns to m2 lines, that an m3 line keeps Revit's deduction) and gives unclassified lines of fewer elements as both. On Revit 2027.2, with walls, floors and roofs layered, no element on either sample falls back to whole; one Snowdon wall whose computed area Revit leaves empty, measured whole with no metrado, is measured by its three materials, which reconcile to its volume. No sample material has a Keynote, so every layer line is unclassified. Structure in m3 makes 43 Snowdon elements and one Pacific floor fall back on a material spanning two units. The export took 3.6 s whole and 6.5 s layered on Snowdon, 1.2 and 1.6 s on Pacific. These runs were made before the second review's wording changes to the layer warnings, which change no count.
