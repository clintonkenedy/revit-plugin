...
The header's `name` is required; `sharedParameter` is the GUID of the shared parameter the codification chain reads third, or null, which settles where the estimator nominates it. The writer states every field of every category, layers as all seven functions or `false`, so what a file reproduces never depends on the built-in criteria it is read over. Reading goes through the criteria file's parser and merge, with the same located refusals, and a configuration read back equals the one written; run over an unchanged model, it reproduces every metrado bit for bit. 
**Picking and saving a configuration (task 3.7, PR 36; decided by the user).** A configuration picked in Revit is **copied beside the model as its criteria file**, so the choice stays with the model and every export of it uses it; one is **saved through Revit's Save As dialog**, its name the file's. The ribbon gains "Load configuration", which reads the chosen file as a configuration (refusing one that is not, or that the export would refuse, located, and writing nothing) and replaces an existing criteria file only with the estimator's consent, and "Save configuration", which saves the criteria an export of the model would run under, refused the same way. The criteria file beside the model now carries the header into the criteria in force: the export's chain reads the nominated shared parameter third and extraction reads it on every element and material, and the completion dialog names the configuration and the shared parameter. Both commands are `ReadOnly`, writing files and never the document; their file logic is `ConfigurationFiles`, tested apart from Revit, and the seven file-dialog members they use are reviewed into the read-only tripwire. On Revit 2027.2, a configuration placed beside each sample, with walls layered and a GUID the samples do not carry, is named in the dialog with its shared parameter, and gives the same wall rows as PR 32's layered run. Revit's file dialogs themselves cannot be driven by the harness.
...
**Measurement (task 3.2).** An element is measured either wholly by layer or wholly whole, never in part: before any layer line exists, every layer, a membrane included, must be attributed to a material Revit measures and every measured material to a layer, each material must have one unit, and the materials' volumes must add up to the whole volume within **1e-6 m3** (fixed, as PR 25 learnt a slack growing with size lets errors through; 285 times the rounding bound, 3,100 times below the smallest real material seen). Otherwise the element is measured whole, as before, with a warning naming the fault. Each opening is decided once, at the host's threshold. **An opening added back returns to each m2 line its share, the opening's area times the material's layer count**, where no condition holds. **An m3 line keeps Revit's deduction** of every opening, as does every line under a condition, and one warning names the openings and those lines: it tells a line in m3 about how much it keeps (the openings' area times its layers' width), and under a condition it states no amount, the condition being that none can be told. PR 28 decided this on ground truth for walls and floors (below): the area share is exact, the volume share is not.
...
**The share, held to deletion (PR 28).** The probe `probe-layer-openings` deletes each opening Metrado measures in a layered wall, floor or roof in force, in a transaction that is rolled back, and reads what the host and each material get back. Its first run stopped at the harness's cap of 150 inside Snowdon's walls, never reaching a floor; the cap now applies per category, and the run was repeated with none reached. Over **220 Snowdon openings (207 in walls, 13 in floors) and 35 Pacific ones (all in walls)**, **each material's area gets back the host's gain times its layer count to 3e-13 m2**, far inside the gate the design set (1e-6); its **volume gets back the gain times its summed width only to 0.08% where no condition holds (0.31% on the foundation opening below) and 0.57% under one**, on either side, so a volume share would sometimes add back material Revit did not remove, and m3 lines keep the deduction instead. The probe cannot delete the 11 holes Snowdon's floors draw with their own outline; for them the share rests on the floors' areas, every floor material's area being its layer count times the floor's to 4e-16 of it (PR 27's `probe-materials`, ten of those eleven floors among them; the eleventh, a one-layer 6" concrete slab, is not, the probe keeping types of more than one layer). No sample roof has a measured opening, so for roofs the share is argued (a cut through parallel layers removes its area from each), not probed. Eleven Pacific openings in walls that wrap at inserts held the area share exactly, and a negative control that set a type to wrap changed no material's gain on either sample; why is not known, the probe recording nothing of the inserts, so wrapping is kept as a condition on argument, not evidence. The same run found one wall opening, a rectangular opening in a Snowdon 24" foundation wall, measured at 21.74 m2 by the wall rule of PR 16 while deleting it gives back 19.94 m2: an overstatement that far above any threshold changes no metrado, but the rule allows none, and its cause (probably a cut hidden inside the opening, as for floors) is open.

**The workbook (task 3.5, PR 31).** A layer line keeps its host's `UniqueId` and fills two columns a whole element's line leaves blank: **Material**, the material's name, and **Layers**, the layers it covers, exterior (or top) first, each as its function, spelled as in the criteria file, and its width in millimetres (`Finish1 15 mm + Finish2 15 mm`), written invariant so a machine set to a decimal comma writes the same text; a layer given the category's material says so. The Unclassified sheet names an uncoded line's material, the thing to key, and is headed "Unclassified lines", the count it states: a layered element with two unkeyed materials is two lines. Lines of one host are ordered by the first layer of their material, which is total: each position holds one layer and each layer one material. The golden `material-layers.xlsx` freezes four layered walls: the spec's three-material wall, one material over both faces, two plasters of one host in one partida and two uncoded materials, each pair chosen so that name order, width order either way and last-layer order all disagree with the first layer's.

**The export (PR 32).** The command asks extraction for the layers of the categories the criteria take off by layer, none without a criteria file. The export measures each of their elements by its materials, each line coded by its material, or, when its materials do not account for it, whole under its own code with the reason; an opening near the threshold is flagged once for the host. A layer its type leaves without a material is measured as the category's material ("Default Wall" on Snowdon's Alley wall), and the run says so once for the type, counting the layers from 1 from the exterior; the budget sheet's Layers column marks it too, but only on a coded line, and on the samples a category material is never coded. The dialog describes a layered criterion (which functions are in m3, how an added-back opening returns to m2 lines, that an m3 line keeps Revit's deduction) and gives unclassified lines of fewer elements as both. On Revit 2027.2, with walls, floors and roofs layered, no element on either sample falls back to whole; one Snowdon wall whose computed area Revit leaves empty, measured whole with no metrado, is measured by its three materials, which reconcile to its volume; but Revit gives each of them an area of 0, so its three m2 lines are 0 m2 (warned about from PR 33). No sample material has a Keynote, so every layer line is unclassified. Structure in m3 makes 43 Snowdon elements and one Pacific floor fall back on a material spanning two units. The export took 3.6 s whole and 6.5 s layered on Snowdon, 1.2 and 1.6 s on Pacific. These runs were made before the second review's wording changes to the layer warnings, which change no count.
