...
.DESCRIPTION
    ...
    The new folder is staged beside the old one and swapped in, and the
    manifest is removed first and written last, so Revit never finds a
    manifest pointing at a half-copied folder. Nothing is replaced that this
    script did not deploy: a Metrado\ folder without Metrado.Revit2027.dll,
    or a Metrado.addin registering another add-in, stops the run untouched.
#>
[CmdletBinding()]
param(
...
try {
    ...
    Copy-Item (Join-Path $publish 'Metrado.addin') $manifest
}
finally {
