...
public sealed class HarnessReport
{
    ...
    public LayerOpeningProbe.Result? LayerOpeningProbe { get; set; }
    ...
    private void OnIdling(object? sender, IdlingEventArgs e)
    {
        ...
        try
        {
            if (!_report.CommandPosted)
            {
                ...
                if (string.Equals(_request.Mode, "probe-layer-openings", StringComparison.OrdinalIgnoreCase))
                {
                    Write("probing");
                    _report.LayerOpeningProbe = LayerOpeningProbe.Run(document, _request.MaxWalls);
                    _report.ModifiedAfterCommand = document.IsModified;
                    Finish(app, "done");
                    return;
                }
                ...
                if (string.Equals(_request.Mode, "probe-materials", StringComparison.OrdinalIgnoreCase))
                {
                    Write("probing");
                    _report.MaterialProbe = MaterialProbe.Run(document, _request.MaxWalls);
                    _report.ModifiedAfterCommand = document.IsModified;
                    Finish(app, "done");
                    return;
                }
